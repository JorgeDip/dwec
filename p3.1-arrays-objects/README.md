...
> **Student Name**: Jorge Rodríguez Rodríguez
...
1. `getStudentByName`: Return a student by name.
1. `removeStudentByName`: Removes a student by filtering out their name from the array.
1. `updateStudentGrades`: Finds a student by name and updates their grades.
1. `calculateAverageGrade`: Returns the average of a student's grades with a given name
1. `getEnrolledStudents`: Returns an array of students who are enrolled.
1. `getStudentsAboveAge`: Returns students older than a given age.
1. `getTopStudent`: Finds the student with the highest average grade.
1. `getStudentSummaries`: Maps the students into a summary object with name and average grade.
1. `getTopStudents`: Filters students by a grade threshold.
1. `getEnrolledStudentNames`: Filters enrolled students and returns an array of their names.
1. `formatGrades`: Returns an array of strings formatting each student's grades.
1. `getHonorRollStudents`: Filters students whose average grade is 90 or above.
1. `serializeStudents`: Converts the students array to a JSON string.
1. `deserializeStudents`: Converts a JSON string back into the students array.
